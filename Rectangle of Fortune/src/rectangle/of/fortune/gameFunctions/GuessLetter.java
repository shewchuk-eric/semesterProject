        ...
        while (true) {
            ...
            for (char secretLetter : secretWord.toCharArray()) {//iterates over the letters
                if (guesses.indexOf(secretLetter) == -1) { //checks to see what letters that have already been guessed are in the secret word 
                    displayWord += "_ ";//prints out underscores with a space between each one to show missing letters
                    ...
                    displayWord += (secretLetter + " "); //If there are some guessed letters, it will display the guessed letters with a space next to them (for aesthetic purposes)
                }
            }
